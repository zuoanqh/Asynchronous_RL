...
def create_model(num_actions, agent_history_length, resized_width, resized_height):
    with tf.device("/cpu:0"):
        ...
        model = TimeDistributed(Convolution2D(nb_filter=16, nb_row=8, nb_col=8, subsample=(4, 4), activation='relu'))(
            inputs)
        model = TimeDistributed(Convolution2D(nb_filter=32, nb_row=4, nb_col=4, subsample=(2, 2), activation='relu'))(
            model)
...
class A3C_LSTM:
    ...
    def create_operations(self, num_actions):
        ...
        for i in range(FLAGS.num_concurrent):
            ...
            self.update_policy.append(
                [self.p_params[i][j].assign(self.policy_model_params[j]) for j in range(len(self.p_params[i]))])
            self.update_value.append(
                [self.v_params[i][j].assign(self.value_model_params[j]) for j in range(len(self.v_params[i]))])
        ...
        p_loss = -tf.reduce_sum(
            tf.reduce_sum(log_probs * self.actions, [1]) * tf.stop_gradient(advantage)) - FLAGS.BETA * entropy
    ...
    def actor_learner_thread(self, env, thread_id, num_actions):
        ...
        while self.T < self.TMAX:
            ...
            while not (done or ((t - t_start) == t_max)):
                ...
                probs = self.session.run(self.local_policy[thread_id],
                                         feed_dict={self.local_states[thread_id]: [state_sequence]})[0]
            ...
            if done:
                ...
                R_t = self.session.run(self.local_value[thread_id],
                                       feed_dict={self.local_states[thread_id]: [state_sequence]})[0][0]
